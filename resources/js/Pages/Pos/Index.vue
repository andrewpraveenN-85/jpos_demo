<template>
    ...
    <p class="text-3xl font-bold tracking-wide text-black">
                            ...
                            <div v-else>
                               <div
                                  ...
                                  <div
                                     ...
                                     class="p-4 shadow border-2 bg-gray-300 border-gray-100 text-xl text-center text-blue-600 cursor-pointer flex flex-col items-center justify-center"
                                     >
                                     <!-- Order ID -->
                                     <p class="font-bold">Order ID: {{ order.order_id }}</p>
                                     <!-- Return Date (Only Displays if Exists) -->
                                     <div
                                        v-if="order.return_date && order.return_date !== ''"
                                        class="mt-2   text-black text-sm font-bold rounded"
                                        >
                ...
                <div class="flex w-1/2 p-8 border-4 border-black rounded-3xl ">
                   <div class="flex flex-col items-start justify-center w-full px-12 pb-0"  >
                      <div class="flex items-center justify-between w-full">
                         <div v-if="selectedOrder && selectedOrder.status == 1" class="mb-3 flex items-center">
                            <input type="checkbox"
       id="returnBill"
       ...
       class="h-8 w-8 text-black-600 border-black rounded focus:ring focus:ring-black-500">
                            ...
                            <label for="returnBill" class="ml-2 text-2xl font-bold tracking-wide text-black">Return Bill</label>
                         </div>
                      </div>
                      <div class="flex items-center justify-between w-full">
...
>
   ...
   <div class="flex flex-col justify-between w-5/6">
    ...
    <p v-if="selectedOrder?.sale_items?.[index]?.return_reason?.reason && selectedOrder?.sale_items?.[index]?.return_reason?.reason !== 'No Reason'" class="text-black font-bold">
  Reason: {{ selectedOrder?.sale_items?.[index]?.return_reason?.reason }}
</p>


      <p class="text-xl text-black">{{ item.name }}</p>
      ...
      <div v-if="isReturnBill && !selectedOrder?.sale_items?.[index]?.reason_id" class="mt-2">
    <select v-model="item.returnReason" class="w-full border border-gray-400 p-2 rounded">
        <option value="" disabled>Select a reason</option>
        <option v-for="reason in props.returnReasons" :key="reason.id" :value="reason.id">
            {{ reason.reason }}
        </option>
    </select>
</div>
 ...
 <script setup>
    ...
    //       returnReason: item.reason_id || "",
    //       orderId: response.data.order_id,
    //     }));


    //     cash.value = response.data.cash || 0;
    //       custom_discount.value = response.data.custom_discount || 0;
    //       discount.value = response.data.discount || 0;
    //       selectedStatus.value = response.data.status?.toString() || "";
    //       order_id.value = response.data.order_id || "";

    //     isReturnBill.value = products.value.some((item) => item.returnReason !== "");
    ...
    const fetchOrderDetails = async (orderId) => {
      try {
        ...
        if (response.data) {
          selectedOrder.value = response.data;
          isReturnBill.value = response.data.is_return_bill == 1;
          products.value = response.data.sale_items.map((item) => ({
            saleItemId: item.id,
            sale_id: item.sale_id,
            product_id: item.product_id,
            reason_id: item.reason_id || "N/A",
            quantity: item.quantity,
            unit_price: item.unit_price,
            total_price: item.total_price,
            product_name: item.product?.name || "Unknown Product",
            product_image: item.product?.image ? `/${item.product.image}` : "/images/placeholder.jpg",
            discount: item.product?.discount || 0,
          }));

        } else {
          console.error("No data found for order");
        }
      } catch (error) {
        console.error("Error fetching order details:", error);
        selectedOrder.value = null;
    ...
    onMounted(() => {
      fetchPendingOrders();
        // console.log('Return Reasons:', props.returnReasons);
    });

    watch(selectedOrder, (newOrder) => {
    if (newOrder) {
           isReturnBill.value = newOrder.is_return_bill == 1;

        // Only enable return bill if status is 1 (Completed order)
