<template>
                ...
                <div class="flex flex-col w-1/2">
                   <div class="flex flex-col w-full">
                      <div class="flex flex-col items-center justify-center w-full pb-4 space-y-8">
                         <div
                            class="flex flex-col items-center justify-center w-full space-y-8 border-4 border-black rounded-3xl p-4">
                            <p class="text-4xl font-bold text-black ">Pending Orders
                               <button @click="resetToLiveBill"
                            ...
                            <div v-else>
                               <div class="grid grid-cols-4 gap-4 w-full h-[400px] overflow-y-auto"
                                  ...
                                  <div v-for="order in pendingOrders" :key="order.order_id"
                                     ...
                                     class="p-4 shadow border-2 bg-gray-300 border-gray-100 text-xl text-center text-blue-600 cursor-pointer flex-col items-center justify-center">
                                     <p class="font-semibold text-lg text-black">Order ID:

                                        <span class="text-orange-600">{{ order.order_id }}</span>
                                       </p>

    <p v-if="order.customer_name !== 'Guest'" class="text-black font-semibold text-lg">
        Customer: <span class="text-blue-600">{{ order.customer_name }}</span>
    </p>

    <p v-if="order.employee_name !== 'Not Assigned'" class="text-black font-semibold text-lg">
        Employee: <span class="text-green-600">{{ order.employee_name }}</span>
    </p>
                                  </div>
 ...
 <script setup>
    ...
    const fetchPendingOrders = async () => {
        try {
            ...
            pendingOrders.value = response.data.map((order) => ({
                ...order,
               customer_id: order.customer_id || "N/A",  // Include customer name
               employee_id: order.employee_id || "N/A",  // Include customer name
            }));
    ...
    const calculateWarrantyPeriod = (start, end) => {
        ...
        // Create result object excluding null values
        const result = {};
    ...
    const fetchOrderDetails = async (orderId) => {
        try {
            ...
            selectedOrder.value.customer_id = response.data.customer_id || "N/A";
            selectedOrder.value.employee_id = response.data.employee_id || "N/A";
