...
const downloadPDF2 = () => {
  ...
  const tableData = Object.entries(productQuantities.value).map(([product, quantity]) => [
    product,
    quantity,
  ]);
...
const downloadPDF3 = () => {
  ...
  const tableData = Object.entries(paymentMethodTotals.value).map(([method, total]) => [
    method,
...
const downloadPDF4 = () => {
  ...
  const tableData = Object.entries(sortedProductsStock.value).map(([product, stock]) => [
    product,
    `${stock} units`,
  ]);
